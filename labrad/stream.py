    ...
    if isinstance(records, str):
        ...
        def genexp():
            for idx, rec in enumerate(records):
                try:
                    yield flattenRecord(*rec, **kw)
                except T.FlatteningError as e:
                    e.err_idx = idx
                    raise
        data = ''.join(genexp())
        #data = ''.join(flattenRecord(*rec, **kw) for rec in records)
    buf = T.OutputStream(endianness=endianness)
    PACKET_TYPE.__flatten__(buf, (context, request, target, data))
    return str(buf.tobytes())
...
def flattenRecord(ID, data, types=[], endianness='>'):
    ...
    try:
        ...
        raise
    buf = T.OutputStream(endianness=endianness)
    RECORD_TYPE.__flatten__(buf, (ID, str(t), str(s)))
    return str(buf.tobytes())

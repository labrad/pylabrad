...
def hexstr_to_int(s):
    """Convert a string of hex digits to an int after stripping whitespace."""
...
def bytes_to_int(bs):
    """Convert a big-endian byte string to an int."""
    n = 0
    for b in bs:
        n = (n << 8) + ord(b)
    return n
...
def hash_all(hash_class, *args):
    """Concatenate and hash the given args using the given hash function.

    hash_class: a callable that returns a hash algorithm with the same
        interface as the hash algorithms defined in hashlib.

    args: Each arg can be given as a byte string or an int. Integer args are
...
# For details see RFC 5054 (https://tools.ietf.org/html/rfc5054)

SrpGroup = namedtuple('SrpGroup', ['N', 'g'])
...
class SRP(object):
    ...
    For details of the algorithm, see https://tools.ietf.org/html/rfc5054
    """
    def __init__(self, identity, password, group=SrpGroup1024,
                 hash_class=hashlib.sha1):
        """Initialize an SRP instance.

        Args:
            identity (str): username
            password (str): password
            group (SrpGroup | str): Group parameters to use in the SRP protocol
                or name of a group to use.
            hash_class (callable): Called with no args and should return a hash
                algorithm with the same interface as those defined in hashlib.
        """
        if isinstance(group, str):
            group = srp_groups[group]
        ...
        def generator():
            ...
            # send identity. get back salt and server B
            salt, B_bytes = yield

            a = bytes_to_int(os.urandom(32))
            A = pow(g, a, N)

            B = bytes_to_int(B_bytes)
            if B % N == 0:
                raise Exception("auth failed: invalid challenge from server")

            x = bytes_to_int(H(salt, H(identity, ':', password)))
            k = bytes_to_int(H(N, PAD(g)))
            ...
            M1 = bytes_to_int(H(PAD(A), PAD(B), PAD(S)))
            M2 = bytes_to_int(H(PAD(A), PAD(M1), PAD(S)))

            # send client confirmation M1, get back server confirmation M2
            M2_server_bytes = yield int_to_bytes(A), int_to_bytes(M1)
            M2_server = bytes_to_int(M2_server_bytes)

            # authentication succeeded if the server M2 matched ours
            yield M2_server == M2

        self._generator = generator()
        self._generator.next()

    def process_server_challenge(self, salt, B_bytes):
        A_bytes, M1_bytes = self._generator.send((salt, B_bytes))
        return A_bytes, M1_bytes

    def process_server_confirmation(self, M2_server_bytes):
        success = self._generator.send(M2_server_bytes)
        if not success:
            raise Exception("auth failed: unable to confirm server message")
